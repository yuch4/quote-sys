...
- ページネーション実装
  ...
  - 発注候補一覧: Client Component + useSearchParams + クライアントサイドページネーション（20件/ページ）
...
### ✅ Phase 11: 高度なレポート機能 - **完了**
- 期間フィルター機能
  - 月次・四半期・年次・カスタム期間選択
  - DatePickerコンポーネント（shadcn/ui Calendar + Popover）
  - date-fns による日付操作（startOfMonth, endOfMonth, etc.）
  - 期間タイプ切替で自動日付設定
  - 選択期間に応じた動的データフィルタリング
- 顧客別売上ランキング
  - 顧客別売上・粗利集計
  - 横棒グラフで上位10社を可視化
  - 詳細テーブル（上位20社）
...
- 案件カテゴリ別分析
  - カテゴリ別売上・粗利集計
  - 円グラフで売上構成比を可視化（COLORS配列で色分け）
  - 詳細テーブルでカテゴリ別実績表示
  - 構成比計算・表示
- CSVエクスポート機能
  - 案件別詳細データのCSV出力
  - BOM付きUTF-8でExcel互換性確保
  - ファイル名に日時タイムスタンプ
  - Toast通知でダウンロード完了表示
- レポート画面拡張
  - 月次売上推移グラフを期間対応に変更
  - 顧客別実績テーブル追加（/dashboard/reports）
  - カテゴリ別実績テーブル追加
  - 案件別詳細テーブルにカテゴリ列追加
  - 全テーブルに overflow-x-auto でレスポンシブ対応
- 新規コンポーネント
  - DatePicker（/components/ui/date-picker.tsx）
  - Calendar（/components/ui/calendar.tsx - shadcn/ui）
  - Popover（/components/ui/popover.tsx - shadcn/ui）
...
### Phase 14: グループ会社CRM拡張（優先度：中）
- [x] グループ会社マスタの拡張（企業属性・担当者情報・システム導入状況）
- [x] 会社別システム棚卸モジュール（カテゴリ別の利用システム登録・更新ワークフロー）
- [x] 共通化候補分析ダッシュボード（採用率ヒートマップ、ベンダー別集計、標準化提案）
- [x] コスト最適化シミュレーション（ライセンス数・年間費用を基にした試算機能）
- [x] システム利用情報の更新月入力改善（月選択 → DATEカラムへ自動補正）
- [x] 削減インサイトでグループ会社名を表示
- [x] グループ会社登録フォームの国・関係ステータス入力改善
- [x] システム利用モーダルの契約形態選択と買い切り対応
- [x] システム利用モーダルの契約形態と更新月のレイアウト修正
- [x] グループ会社CRMを独立ページ化しサイドバーに追加
- [x] グループ会社詳細ドロワーに基本情報編集タブを統合
- [x] セキュリティ統制モーダルの「統制種別」を自由入力できるよう対応
- [x] セキュリティ統制登録後もタブが切り替わらないよう挙動を修正
- [x] グループ会社一覧で利用システム/セキュリティ製品によるフィルターを追加
- [x] グループ会社CSVインポート/エクスポート対応
- [ ] 権限制御と監査ログ（グループ会社担当者単位の閲覧/更新範囲制御）
...
- ✅ ビジュアルレイアウトエディタ（VisualLayoutEditor）
- ✅ リアルタイムHTMLプレビュー（LayoutPreview）
- ✅ ビジュアル/テーブル編集モード切替
- ✅ セクション表示/非表示トグル
- ✅ セクション幅スライダー調整
- ✅ ズームコントロール（30-100%）
- ✅ ページ設定情報パネル
- ✅ 型定義拡張（positionMode, x, y, height, zIndex）

**Phase 15で実装完了した機能** (PDFレイアウトカスタマイズ強化):
- ✅ ページ設定（用紙サイズ: A4/A3/Letter/Legal、向き: 縦/横、余白調整）
- ✅ スタイル設定（フォントサイズ、カラー、罫線、スペーシング）
- ✅ テーブルスタイル設定（ヘッダー揃え、縞模様、罫線表示）
- ✅ 会社ロゴ設定（URL、サイズ、位置）
- ✅ ページ番号設定（表示/非表示、位置）
- ✅ セクション・列ごとのカスタムスタイル対応
- ✅ 新設定UIコンポーネント（DocumentLayoutEditor）
- ✅ データベースマイグレーション（page, styles, table_styles カラム追加）

**Phase 11で実装完了した機能**:
- ✅ 期間フィルター（月次・四半期・年次・カスタム）
- ✅ 顧客別売上ランキング（グラフ・詳細テーブル）
- ✅ 案件カテゴリ別分析（円グラフ・詳細テーブル）
- ✅ CSVエクスポート機能
- ✅ DatePickerコンポーネント（date-fns統合）
- ✅ 期間対応の動的データ集計

**次の優先実装（Phase 12-13）**:
- テスト実装（Vitest単体テスト、Playwright E2Eテスト）
- 本番リリース準備（Vercel/Supabaseデプロイ、セキュリティチェック）
