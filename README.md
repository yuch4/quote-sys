...
- **発注管理**: 発注候補一覧・発注書作成・入荷登録・進捗ダッシュボード
...
- **発注候補一覧**: 承認済み見積から仕入要=trueの明細を自動抽出
...
- **顧客別売上ランキング**: トップ顧客の可視化（グラフ・テーブル）
- **カテゴリ別分析**: 案件カテゴリごとの売上構成比（円グラフ）
- **案件別詳細**: 案件ごとの粗利分析
- **計上状況**: 未計上・申請中・計上済みの件数
- **期間フィルター**: 月次・四半期・年次・カスタム期間で分析
