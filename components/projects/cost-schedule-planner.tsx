...
import {
  COST_SCHEDULE_STATUSES,
  type CostScheduleDraft,
  generateEvenlyDistributedCostSchedule,
  sumCostScheduleAmounts,
} from '@/lib/projects/cost-schedule'

type CostPlannerChangeHandler = (state: {
  enabled: boolean
  rows: CostScheduleDraft[]
}) => void

interface MonthlyCostPlannerProps {
  expectedAmount: number
  defaultStartMonth?: string
  initialRows?: CostScheduleDraft[]
  initialEnabled?: boolean
  disabled?: boolean
  onChange?: CostPlannerChangeHandler
}
...
const generateLocalId = () => `cost-${Math.random().toString(36).slice(2, 9)}-${Date.now().toString(36)}`
...
export function MonthlyCostPlanner({
  expectedAmount,
  defaultStartMonth,
  initialRows = [],
  initialEnabled,
  disabled = false,
  onChange,
}: MonthlyCostPlannerProps) {
  const [enabled, setEnabled] = useState(initialEnabled ?? initialRows.length > 0)
  const [config, setConfig] = useState({
    startMonth: initialRows[0]?.costMonth || defaultStartMonth || '',
    months: initialRows.length || 12,
    costDay: initialRows[0]?.costDate ? Number(initialRows[0].costDate.slice(8, 10)) || 25 : 25,
    totalAmount: initialRows.length > 0 ? sumCostScheduleAmounts(initialRows) : expectedAmount || 0,
  })
  const [rows, setRows] = useState<CostScheduleDraft[]>(initialRows)
  ...
  const hydrateFromInitialRows = useCallback(() => {
    if (initialRows.length > 0) {
      ...
      setConfig((prev) => ({
        startMonth: initialRows[0]?.costMonth || prev.startMonth || defaultStartMonth || '',
        months: initialRows.length,
        costDay: initialRows[0]?.costDate ? Number(initialRows[0].costDate.slice(8, 10)) || prev.costDay : prev.costDay,
        totalAmount: sumCostScheduleAmounts(initialRows),
      }))
      setTotalDirty(true)
      setHydrated(true)
    }
  ...
  const plannedTotal = useMemo(() => sumCostScheduleAmounts(rows), [rows])
  ...
  const handleGenerate = () => {
    ...
    if (!config.totalAmount || config.totalAmount <= 0) {
      setPlannerError('仕入原価（総額）を入力してください')
      return
    }

    const generated = generateEvenlyDistributedCostSchedule({
      startMonth: config.startMonth,
      months: config.months,
      totalAmount: config.totalAmount,
      costDay: config.costDay,
    })
  ...
  const updateRow = (localId: string, changes: Partial<CostScheduleDraft>) => {
  ...
  const handleAddRow = () => {
    if (rows.length === 0) {
      ...
      const seeds = generateEvenlyDistributedCostSchedule({
    ...
    const nextMonth = addMonthValue(lastRow.costMonth) || config.startMonth || defaultStartMonth || ''
    if (!nextMonth) {
      setPlannerError('追加する前に開始月を設定してください')
      return
    }
    ...
    const nextRow = {
      ...
      costMonth: nextMonth,
      costDate: lastRow.costDate ? shiftDateValue(lastRow.costDate) : '',
    }
  ...
  const plannerSummary = useMemo(() => {
    ...
    const diffText = diffFromExpected === 0
      ? '（見込原価と一致）'
      : `（見込原価との差額 ${diffFromExpected > 0 ? '+' : ''}${formatCurrency(diffFromExpected)}）`
    ...
    <div className="space-y-4">
      <div className="flex items-start gap-3 rounded-lg border p-4">
        ...
        <div className="space-y-1">
          <Label htmlFor="auto-cost" className="text-base font-semibold">
            月次仕入計上予定を自動生成
          </Label>
          <p className="text-sm text-gray-500">
            仕入原価を指定した月数に均等配分し、対象月ごとに日付や金額を微調整できます。
          </p>
        </div>
      </div>
      ...
      {enabled && (
        <div className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="costDay">計上予定日（毎月）</Label>
              <Input
                id="costDay"
                type="number"
                min={1}
                max={31}
                value={config.costDay}
                onChange={(event) => setConfig((prev) => ({ ...prev, costDay: Number(event.target.value) }))}
              />
            </div>
            <div className="space-y-2">
              <Label htmlFor="totalAmount">
                按分対象の総額
                <span className="ml-2 text-xs text-gray-500">(仕入原価が自動反映)</span>
              </Label>
              <Input
                id="totalAmount"
                type="number"
          ...
          {rows.length === 0 ? (
            <div className="rounded-lg border border-dashed p-6 text-sm text-gray-500">
              プレビューを生成すると、対象月ごとの仕入計上予定日・金額をここで確認できます。
              行を追加して端数や特別対応月だけを個別に調整することも可能です。
            </div>
          ) : (
            <div className="space-y-2">
              <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                <p className="text-sm text-gray-600">仕入計上予定合計: <span className="font-semibold text-gray-900">{plannerSummary}</span></p>
              </div>
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead className="w-[160px]">計上予定日</TableHead>
                      ...
                      <TableRow key={row.localId}>
                        <TableCell>
                          <Input
                            type="month"
                            value={row.costMonth}
                            onChange={(event) => updateRow(row.localId, { costMonth: event.target.value })}
                          />
                        </TableCell>
                        <TableCell>
                          <Input
                            type="date"
                            value={row.costDate}
                            onChange={(event) => updateRow(row.localId, { costDate: event.target.value })}
                          />
                        </TableCell>
                        <TableCell>
                          <Input
                        ...
                        <TableCell>
                          ...
                          >
                            ...
                            <SelectContent>
                              {COST_SCHEDULE_STATUSES.map((status) => (
